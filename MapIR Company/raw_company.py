...
COLOR_CORRECTION_VECTORS = [1.398822546, -0.09047482163, 0.1619316638, -0.01290435996, 0.8994362354, 0.1134681329,
...
def Survey3_Convert_RAW_to_Tiff(infolder, outfolder, wb):
    ...
    if numFiles > 1:
        first_files_jpg = "JPG" in infiles[0].upper() and "JPG" in infiles[
            1].upper()  # returns true if first two files are JPG
        first_files_rawjpg = "raw" in infiles[0].upper() and "JPG" in infiles[
            1].upper()  # returns true if first two files are raw and JPG
                ...
                if first_files_rawjpg:
                    try:
                        ...
                        udata = np.packbits(np.concatenate(
                            [data[0::3], np.array([0, 0, 0, 0] * 12000000, dtype=np.uint8).reshape(12000000, 4),
                             data[2::3], data[1::3]], axis=1).reshape(192000000, 1)).tobytes()
